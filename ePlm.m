function P=ePlm(varargin)

% Defined by Biaoyan Hu on 241105
...
% P_=sign(x).*P;
% P(jm1_)=P_(jm1_);
% signx=2*(x>=0)-1; % 250309
% P_=(-1).^((signx.*2*l+4*l+1).*absm).*fac(l+absm)./fac(l-absm).*P;
P_=(-1).^absm.*fac(l+absm)./fac(l-absm).*P;
P(jm1_)=P_(jm1_);
% P(jm1_&x<0&l~=round(l))=-P(jm1_&x<0&l~=round(l));
